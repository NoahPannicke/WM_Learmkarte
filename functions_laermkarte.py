def map_func(uhrzeit):
    ...
    # PLZ
    addr_fp = r"https://tsb-opendata.s3.eu-central-1.amazonaws.com/plz/plz.shp.zip"
    plz_db = r"https://raw.githubusercontent.com/NoahPannicke/WM_Learmkarte/main/plz_db.csv"

    # LOR
    lor_shap = r"https://tsb-opendata.s3.eu-central-1.amazonaws.com/lor_planungsgraeume_2021/lor_planungsraeume_2021.shp.zip"
    lor_db_data = r"https://raw.githubusercontent.com/NoahPannicke/WM_Learmkarte/main/lor_db.csv"

    # Streets
    str_shap = r"https://tsb-opendata.s3.eu-central-1.amazonaws.com/detailnetz_strassenabschnitte/Detailnetz-Strassenabschnitte.shp.zip"
    str_db_data = r"https://raw.githubusercontent.com/NoahPannicke/WM_Learmkarte/main/str_db.csv"
    ...
    if ('-' in uhrzeit):
        db['zeit'] = pd.to_datetime(db['zeit'], format='%H:%M')
        lor_db['zeit'] = pd.to_datetime(lor_db['zeit'], format='%H:%M')
        #str_db['zeit'] = pd.to_datetime(str_db['zeit'], format='%H:%M')

        db = db.set_index('zeit')
        lor_db = lor_db.set_index('zeit')
        #str_db = str_db.set_index('zeit')

        db = db.between_time(uhrzeit[:5], uhrzeit[len(uhrzeit) - 5:len(uhrzeit)])
        lor_db = lor_db.between_time(uhrzeit[:5], uhrzeit[len(uhrzeit) - 5:len(uhrzeit)])
        #str_db = str_db.between_time(uhrzeit[:5], uhrzeit[len(uhrzeit) - 5:len(uhrzeit)])

        db = db.reset_index(drop=True)
        lor_db = lor_db.reset_index(drop=True)
        #str_db = str_db.reset_index(drop=True)

        db = db.groupby(['plz'],as_index=False).mean()
        lor_db = lor_db.groupby(['lor'],as_index=False).mean()
        #str_db = str_db.groupby(['str']).mean()
        ...
        # Use recent data for live map
        db = db.sort_values('zeit').groupby('plz', as_index=False).tail(1)
        lor_db = lor_db.sort_values('zeit').groupby('lor', as_index=False).tail(1)
